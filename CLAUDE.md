...
为确保远程部署的完整性和一致性，必须遵循以下推送规则：

1. **完整文件夹推送**：
   - 每次部署时必须推送整个配置文件夹，而非单个文件
   - 使用 `scp -r . root@<host>:/tmp/nixos-config/` 命令
   - 避免部分文件推送导致的版本不一致问题
...
3. **部署命令序列**：
   ```bash
   # 1. 完整推送配置文件夹
   scp -r . root@192.168.88.100:/tmp/nixos-config/

   # 2. 部署配置
   ssh root@192.168.88.100 "cd /tmp/nixos-config && nixos-rebuild switch --flake .#vm-nixos"
   ```

4. **故障排除**：
   - 如果遇到文件不存在的错误，首先检查是否完整推送
   - 重新执行完整推送命令
   - 避免逐个文件推送来解决问题

### Git 仓库管理原则

**重要：NixOS 只会使用 Git 仓库中已被跟踪的文件**

1. **文件可见性规则**：
   - NixOS 在 Git 仓库中只会识别和使用已被 `git add` 的文件
   - 未被 Git 管理的文件（新文件、未跟踪文件）会被完全忽略
   - 即使文件存在于文件系统中，NixOS 也"看不到"它们
...
2. **配置文件提交流程**：
   ...
   # 然后才能进行部署
   scp -r . root@<host>:/tmp/nixos-config/
   ```

3. **特殊情况处理**：
   - 如果遇到 "path does not exist" 错误，首先检查文件是否已被 Git 跟踪
   - 使用 `git status` 查看文件状态
   - 使用 `git add .` 添加所有未跟踪的文件
   - 敏感文件应该添加到 `.gitignore` 中，而不是留在未跟踪状态

4. **最佳实践**：
   - 每次创建新配置文件后立即执行 `git add .`
   - 在部署前确保所有重要文件都已提交到 Git
   - 定期检查 `git status` 确认没有遗漏的配置文件
