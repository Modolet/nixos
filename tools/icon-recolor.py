...
# Auth: Nicklas Vraa
# Note: Adapted to run standalone in Nix builds; replaces basic_colormath with
# a small, local color conversion helper.
...
# Minimal LAB conversion/distance ------------------------------------------------
...
def rgb_to_lab(rgb: Tuple[float, float, float]) -> Lab:
    """Convert sRGB color (0-255) to CIE LAB (D65) using the CIE76 formula."""
...
hex_to_lab_dict = {
    "#ffffff": rgb_to_lab(sRGBColor(255, 255, 255)),  # White.
    "#000000": rgb_to_lab(sRGBColor(0, 0, 0)),  # Black.
}
